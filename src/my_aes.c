...
static int my_aes256_keybit (int length) {
	if ( length < 17 )
		return 128;
...
static int my_aes256_create_key(KEYINSTANCE *aes_key,
			     enum encrypt_dir direction, const char *key,
			     int key_length)
{
  int keybit = my_aes256_keybit (key_length);
  uint8 rkey[keybit/8];	 /* The real key to be used for encryption */
  uint8 *rkey_end=rkey+keybit/8; /* Real key boundary */
...
#define AES_USE_KEY_BYTES (AES_USE_KEY_BITS/8)
  ...
  */  
  ...
  if (direction == AES_DECRYPT)
     aes_key->nr = rijndaelKeySetupDec(aes_key->rk, rkey, keybit);
  else
     aes_key->nr = rijndaelKeySetupEnc(aes_key->rk, rkey, keybit);
...
int my_aes256_encrypt(const char* source, int source_length, char* dest,
		   const char* key, int key_length)
{
  KEYINSTANCE aes_key;
  uint8 block[AES256_BLOCK_SIZE];	/* 256 bit block used for padding */
  int rc;			/* result codes */
  ...
  if ((rc= my_aes256_create_key(&aes_key,AES_ENCRYPT,key,key_length))) {
    DEBUG_SUB_FUNCTION_OUT;
    return rc;
  }

  num_blocks = source_length/AES256_BLOCK_SIZE;

  for (i = num_blocks; i > 0; i--)   /* Encode complete blocks */
  {
    rijndaelEncrypt(aes_key.rk, aes_key.nr, (const uint8*) source,
		    (uint8*) dest);
    source+= AES256_BLOCK_SIZE;
    dest+= AES256_BLOCK_SIZE;
  }
...
int my_aes256_decrypt(const char *source, int source_length, char *dest,
		   const char *key, int key_length)
{
  KEYINSTANCE aes_key;
  uint8 block[AES256_BLOCK_SIZE];	/* 256 bit block used for padding */
  ...
  if ((rc=my_aes256_create_key(&aes_key,AES_DECRYPT,key,key_length))) {
    DEBUG_SUB_FUNCTION_OUT;
    return rc;
  }
  ...
  if ((source_length != num_blocks*AES256_BLOCK_SIZE) || num_blocks ==0 ) {
    DEBUG_SUB_FUNCTION_OUT;
    return AES256_BAD_DATA; /* Input size has to be even and at least one block */
  }
  ...
  pad_len = (uint) (uchar) block[AES256_BLOCK_SIZE-1];

  if (pad_len > AES256_BLOCK_SIZE) {
    DEBUG_SUB_FUNCTION_OUT;
    return AES256_BAD_DATA;
  }
  /* We could also check whole padding but we do not really need this */

  memcpy(dest, block, AES256_BLOCK_SIZE - pad_len);
  DEBUG_SUB_FUNCTION_OUT;
  return AES256_BLOCK_SIZE*num_blocks - pad_len;
}
...
int my_aes256_get_size(int source_length)
{
  ...
  return AES256_BLOCK_SIZE*(source_length/AES256_BLOCK_SIZE)+AES256_BLOCK_SIZE;
}
